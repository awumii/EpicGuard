    ...
    public void run() {
        ...
        if (AttackManager.getJoinPerSecond() < Config.joinSpeed && AttackManager.getPingPerSecond() < Config.pingSpeed && AttackManager.getConnectPerSecond() < Config.connectSpeed) {
            if (AttackManager.isUnderAttack()) {
                ...
                Notificator.broadcast("&8(&4Blacklist&8) &7Changed: &6" + (BlacklistManager.IP_BL.size() - HeuristicsTask.getBlacklistInc()) + " &8-> &6" + BlacklistManager.IP_BL.size() + " &8(&c+" + HeuristicsTask.getBlacklistInc() + "&8).");
