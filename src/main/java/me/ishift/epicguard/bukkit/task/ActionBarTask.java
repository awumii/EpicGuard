...
public class ActionBarTask implements Runnable {
    ...
    public void run() {
        ...
        Notificator.action(Messages.prefix + color1 + AttackManager.getConnectPerSecond() + "&7/" + color1 + "cps &8| " + color2 + AttackManager.getPingPerSecond() + "&7/" + color2 + "pps &8| &7Blocked: " + color1 + AttackManager.getTotalBots() + " &8| &7Duration: &e" + HeuristicsTask.getTime() + "sec &8| " + (AttackManager.isUnderAttack() ? "&cAttack Detected" : "&aNo Attack"));
    }
}
